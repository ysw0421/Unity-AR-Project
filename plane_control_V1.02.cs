...
public class plane_control_V1.02 : MonoBehaviour {
    /* 
    ...
    // 법선 벡터를 그리는 함수.
    public void DrawNormalVector(Vector3 NormalVector, Vector3 MeshPlaneCenter, GameObject Cgb)
    {
        ...
        if (line.GetComponent<LineRenderer>() == null)
        {
            ...
            lineRenderer.SetPosition(0, MeshPlaneCenter);
            lineRenderer.SetPosition(1, -NormalVector + MeshPlaneCenter);
        }
        else
        {
            lineRenderer.SetPosition(0, MeshPlaneCenter);
    ...
    {
        GameObject GB = new GameObject("DrawMashPlane");
    ...
    void Start () {
        ...
        line = new GameObject("NormalVector");
        ...
        if (quad2.points != null)
        {
            ...
            */
            ...
            for (int i = 0; i < z1.Length; i++)
            {
                ...
                if (i == 3 && quad2.FindObject == true && quad2.AT == true)
                {
                    //ShowPlane
                    ...
                    // Mesh로 만들어진 Plane의 중점.
                    Vector3 meshplane = new Vector3(quad2.world_x, -quad2.world_y, 500);

                    //필요한 기능.
                    //--------------------------------------------------------------------------------
                    MeshRenderer mr = Cgb.GetComponent(typeof(MeshRenderer)) as MeshRenderer;
                    mr.material.shader = Shader.Find("Unlit/Transparent");
                    mr.material.mainTexture = imgTexture;
                    //--------------------------------------------------------------------------------

                    // 법선 벡터를 렌더링 한다.
                    // DrawNormalVector(NV, meshplane, Cgb);
                ...
                else if (i == 3 && quad2.FindObject == false && quad2.AT == true)
                {
                    // 1. testORB에서 Mat H가 null일 경우 -> CVException
                    // 2. testORB에서 두 대각선이 교차하지 않을 경우 발생.
                    x[0] = Ppoint[1, 0]; x[1] = Ppoint[2, 0]; x[2] = Ppoint[3, 0]; x[3] = Ppoint[4, 0];
                    y[0] = Ppoint[1, 1] + 1000; y[1] = Ppoint[2, 1] + 1000; y[2] = Ppoint[3, 1] + 1000; y[3] = Ppoint[4, 1] + 1000;
                    z[0] = 500 - z1[i]; z[1] = 500 - z2[i]; z[2] = 500 + z1[i]; z[3] = 500 + z2[i];
                    inst.transform.position = new Vector3(0, 700, 500);
                    ShowPlane(x, y, z, i, Cgb);
                }
                else if (quad2.AT == false)
                {
                    x[0] = 0; x[1] = 0; x[2] = 0; x[3] = 0;
                    y[0] = 700; y[1] = 700; y[2] = 700; y[3] = 700;
                    z[0] = 500 - z1[i]; z[1] = 500 - z2[i]; z[2] = 500 + z1[i]; z[3] = 500 + z2[i];
                    ShowPlane(x, y, z, i, Cgb);
                    inst.transform.position = new Vector3(0, 700, 500);
                }
